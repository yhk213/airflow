...
with DAG(
    ...
    @task(task_id='task_using_macro',
          templates_dict = {'start_date' : '{{ (data_interval_end.in_timezone("Asia/Seoul") + macros.dateutil.relativedelta.relativedelta(months=-1, day=1)) }}',
                            'end_date' : '{{ (data_interval_end.in_timezone("Asia/Seoul").replace(day=1) - macros.dateutil.relativedelta.relativedelta(days=19)) }}'
          }
    )
